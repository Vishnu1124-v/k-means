...
def load_model():
    ...
    try:
        model = joblib.load("kmeans_random_forest_model.pkl")
        return model
...
if model is not None and df is not None:
    ...
    if st.button("🚀 Predict Cluster", use_container_width=True, key="predict_btn"):
        ...
        with col2_info:
            ...
            temp_df['Cluster'] = kmeans.fit_predict(scaled_data)
            
            cluster_stats = temp_df[temp_df['Cluster'] == cluster]
           
            st.markdown(f"""
                <div class="cluster-info">
                    <p><strong>Size:</strong> {len(cluster_stats)} customers ({len(cluster_stats)/len(temp_df)*100:.1f}%)</p>
        ...
        with viz_col1:
            ...
            # Use the dynamically generated temp_df with clusters
           
            # Inverse transform to original scale for visualization
            original_scaled = temp_df[['Age', 'Annual Income (k$)', 'Spending Score (1-100)']].copy()
            ...
            fig = px.scatter_3d(
                ...
                color=temp_df['Cluster'].astype(str),
...
st.markdown("""
    ...
    <p>🛍️ Mall Customer Clustering Analysis | by VISHNU-1124 </p>
    </div>
""", unsafe_allow_html=True)
